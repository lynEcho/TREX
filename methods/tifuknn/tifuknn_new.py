...
def temporal_decay_sum_history(data_set, key_set, output_size, group_size, within_decay_rate, group_decay_rate):
    sum_history = {}
    for key in key_set: #for each user
...
def evaluate(data_history, training_key_set, test_key_set, input_size, group_size,
    ...
    sum_history, pred_rel_dict = merge_history(temporal_decay_sum_history_test, test_key_set, temporal_decay_sum_history_training,
                                training_key_set, neighbour_index, alpha) #pred
...
def main(argv):
    ...
    pred_path = f'results/'+dataset_name + '_pred0'+'.json'
    pred_rel_path = f'results/'+dataset_name + '_rel0'+'.json'
